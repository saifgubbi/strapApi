...
//    var locType = (req.query.locType || '%') + '%';
    ...
    if (req.query.invDtFrom && req.query.invDtTo) {
        invDt = `AND A.INV_DT BETWEEN '${moment(req.query.invDtFrom).format("DD-MMM-YYYY")}' AND '${moment(req.query.invDtTo).format("DD-MMM-YYYY")}'`;
    }
...
function getSerial(req, res) {
   ...
   if (req.query.part) {
    ...
    var sqlStatement = `SELECT S.SERIAL_NUM,S.SERIAL_DT,S.BIN_LABEL,NVL(SI.PICK_LIST,S.PICK_LIST) PICK_LIST,NVL(SI.BIN_ID,S.BIN_ID) BIN_ID,S.PALLET_ID,SI.LOC_TYPE,SI.INVOICE_NUM FROM SERIAL_T S,SERIAL_INV_T SI
