...
function getId(req, res) {
    ...
    if (objType === 'Bin') {
        sqlStatement = `SELECT BIN_ID AS "objId" , PART_NO as "partNo",QTY as "qty" FROM BINS_T WHERE LABEL = '${objLbl}' AND PART_GRP='${partGrp}' AND ROWNUM=1`;
    } else {
        sqlStatement = `SELECT PALLET_ID AS "objId", PART_NO as "partNo",QTY as "qty" FROM PALLETS_T WHERE LABEL = '${objLbl}' AND PART_GRP='${partGrp}' AND ROWNUM=1`;
    }
